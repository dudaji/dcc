...
**문제 2번**
ASCII CODE 문자열 "Hello world!"의 (A) [byte단위 xor값]과 (B) [byte단위 합계] 를 구하여
상위 8bit는 (A)로 채우고, 하위 24bit는 (B)로 채운 Integer 값을 Hex Value로 출력하시오.

**문제 5번**
어떤 32bit unsigned integer 값을 입력으로 받아, 이것의 모든 비트에 대해 0을 1로 바꾸고 1을 0으로 바꾼 수를 리턴해주는 함수 InverseBits를 작성하고, InverseBits(123456789) 를 출력하는 프로그램을 작성하시오.
...
**문제 6번**
Intel CPU 기반에서, IEEE 32bit floating point는 Mantissa (Significand) 23비트, Exponent 8비트, Sign 1비트로 표현되며, 
IEEE 16bit floating point는 Mantissa (Significand) 10비트, Exponent 5비트, Sign 1비트로 표현된다.

32 bit floating point 의 Mantissa 를 구하는 함수 unsigned int GetMantissa32(float value) 와
16 bit floating point 의 Mantissa 를 구하는 함수 unsigned int GetMantissa16(float value) 를 작성하고, 
GetMantissa32(3.14194989) + GetMantissa16(3.14194989) 의 값을 출력하는 프로그램을 작성하시오.

두 함수는 32bit floating point 를 입력으로 받으며, 리턴값은 각각 최대 2^23-1, 2^10-1 이다.
